...
exports.View = void 0; // this K is because Model is also an generic class that uses a generic like UserProps
...
function () {
  ...
  View.prototype.bindModel = function () {
    var _this = this; // as soon as a new UserForm in instantiated with a model,
  ...
  View.prototype.bindEvents = function (fragment) {
    ...
    var _loop_1 = function _loop_1(eventKey) {
      ...
      var _a = eventKey.split(':'),
          eventName = _a[0],
          selector = _a[1]; // find all the elements inside the html that matches this selector
  ...
  View.prototype.render = function () {
    ...
    this.parent.innerHTML = ''; // creates an template element that holds an valid html string in its innerHTML

    var templateElement = document.createElement('template');
    templateElement.innerHTML = this.template(); // we pass in the DocumentFragment to the bindEvents so bindEvents function will check for
    ...
    this.onRender(); // content property is a reference to a (type of) DocumentFragment (actual html element)
...
var View_1 = require("./View"); // UserProps will be used by the generic Model class :)
...
function () {
  function Model(attributes, events, sync) {
    ...
    this.events = events;
    this.sync = sync; // returns a reference to the on method defined in Eventing class
  ...
  Model.prototype.set = function (update) {
    this.attributes.set(update); // this event trigger will tell other parts of our application something changed if they listen to event called 'change'
...
function () {
  function Attributes(data) {
    ...
    this.data = data; // type of the parameter 'key' this function receives will be of type the key of the Type T corresponding to the 'key' argument
...
exports.Attributes = Attributes; // // Not a variable declarition, it is a type that is 'stephen'
...
var axios_1 = __importDefault(require("axios")); // T extends hasId, so T generic has to be passed in a type that has id (can be optional)
...
function () {
  ...
  ApiSync.prototype.save = function (data) {
    var id = data.id; // presence of id means this user exists in database already, and we can make a put request
...
function () {
  function Eventing() {
    ...
    this.events = {}; // custom event listener
    ...
    this.on = function (eventName, callback) {
      ...
      var handlers = _this.events[eventName] || []; // we push either to an brand new empty array, or the existing array with the key of eventName inside this.events object

      handlers.push(callback); // this.events object property with eventName key is updated with a new array
    ...
    this.trigger = function (eventName) {
      var handlers = _this.events[eventName]; // if there is no key of eventName inside this.events object, or the key has an empty array just return from this function
...
var Eventing_1 = require("./Eventing"); // T is a class instace, K type of structure(interface) the T class has
...
var rootUrl = 'http://localhost:3000/users'; // User is a Model but decides how, and what Attributes,Eventing, and Sync classes to use
...
function (_super) {
  ...
  User.buildUser = function (attrs) {
    return new User( // this satisfies the constructor of the Model class so methods defined in Model class can be used by the User instance
    new Attributes_1.Attributes(attrs), new Eventing_1.Eventing(), new ApiSync_1.ApiSync(rootUrl));
  };
